...
#İki karakteri karşılaştırmak için
...
def findListOfListsOfMatchingChars(listOfPossibleChars):
            ...
            # Bu fonksiyonda mümkün olan karakterlerle tek bir  ve büyük liste yapılır.Amaç Listeyi düzenlemek.

    listOfListsOfMatchingChars = []

    for possibleChar in listOfPossibleChars:                       #Büyük karakter lsitesinde ki olan her bir karakter possiblechar değerini alır.
        listOfMatchingChars = findListOfMatchingChars(possibleChar, listOfPossibleChars)        #Mevcut listedekileri büyük lsitede bulma

        listOfMatchingChars.append(possibleChar)                # also add the current char to current possible list of matching chars
                                                                #eşleşen mevcut karakteri ekle
        # Belirlenen karakter sayısına uygunsa lsiteye ekle
        if len(listOfMatchingChars) < MIN_NUMBER_OF_MATCHING_CHARS:    # Belirlenen karakter sayısına uygunsa lsiteye ekle
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    listOfMatchingChars.sort(key = lambda matchingChar: matchingChar.intCenterX)        # karakterleri soldan sağa sıralar
